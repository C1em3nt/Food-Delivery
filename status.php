          ...
          <div class="col-xs-12">
              ...
              <?php
                ...
                if($q == 'all'){
                    ...
                    $stmt = $conn->prepare("select * from orders where buyer=:buyer and status=:status ORDER by OID ASC");
                    $stmt->execute(array('buyer' => $_SESSION['Account'], 'status' => $q ));
                    $row = $stmt->fetchall();
                }

                foreach($row as $olist){?>
                <form name="orderform<?=$olist['OID']?>" id="orderform<?=$olist['OID']?>" method="post" >
                <tr>
                ...
                <script>
                      function buyercancel<?=$olist['OID']?>() { 
                        orderform<?=$olist['OID']?>.action="buyercancel.php";
                        orderform<?=$olist['OID']?>.submit();
                      }
                  </script>
                </form>
                <?php }?>
