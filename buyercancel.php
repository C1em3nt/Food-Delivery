...
try{
    ...
    $stmt->execute(array('tot_amount' => $order['tot_amount'] ,'account' => $_SESSION['Account']));
    
    $stmt = $conn->prepare("update users set balance = balance - :tot_amount + :fee where account=:account");
    $stmt->execute(array('tot_amount' => $order['tot_amount'], 'fee' => $order['fee'], 'account' => $shop['owner']));
    ...
    $stmt->execute(array('amount' => "+".$order['tot_amount'], 'UID' => $order['buyer'], 'trader' => $order['shop']));

    $stmt = $conn->prepare("insert into transaction (action, amount, time, UID, trader) values ('Payment', :amount, NOW(), :UID, :trader)");
    $stmt->execute(array('amount' => ($order['fee'] - $order['tot_amount']) , 'UID' => $shop['owner'], 'trader' => $order['buyer']));
